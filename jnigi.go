...
// Must call runtime.LockOSThread() first.
func GetJVMs() (int, error) {
	var len jsize = 128

	u := unsafe.Sizeof((unsafe.Pointer)(nil))
	vmBuf := malloc(u * uintptr(len))
	defer free(vmBuf)
	var n jsize

	if jni_GetCreatedJavaVMs(vmBuf, len, &n) < 0 {
		return 0, errors.New("Couldn't get JVMs")
	}
